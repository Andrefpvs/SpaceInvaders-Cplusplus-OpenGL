...
#define VELOCITY 4.5f //4.5 e' o valor ideal para disparar tiros 'nao colados'
...
void Shot::setX(float x){ //em principio este setter nunca sera usado, visto o movimento ser vertical
    _xPos = x;
}
...
void Shot::setRadius(double radius){
	_radius = radius;
}

void Shot::setFired(bool fire){
	_fired = fire;
}
void Shot::drawShot(bool wire){
	if(wire){		
		glPushMatrix();
		GLfloat mat_diffuse[] = { 1.0f, 0, 0 };
		GLfloat mat_specular[] = {  1.0f, 0, 0 };
